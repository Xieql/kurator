...
func (a *ApplicationManager) syncSourceResource(ctx context.Context, app *applicationapi.Application) error {
	...
	case HelmRepoKind:
		targetSource := &sourcev1.HelmRepository{
			ObjectMeta: metav1.ObjectMeta{
				Name:      generateSourceName(app),
				Namespace: app.Namespace,
				Labels: map[string]string{
	...
	case OCIRepoKind:
		targetSource := &sourcev1.OCIRepository{
			ObjectMeta: metav1.ObjectMeta{
				Name:      generateSourceName(app),
				Namespace: app.Namespace,
				Labels: map[string]string{
...
func (a *ApplicationManager) syncResource(ctx context.Context, targetSource client.Object, resourceKind string) error {
	...
	case HelmRepoKind:
		err = a.updateHelmRepository(ctx, currentResource.(*sourcev1.HelmRepository), targetSource.(*sourcev1.HelmRepository))
	case OCIRepoKind:
		err = a.updateOCIRepository(ctx, currentResource.(*sourcev1.OCIRepository), targetSource.(*sourcev1.OCIRepository))
	case KustomizationKind:
...
func (a *ApplicationManager) updateHelmRepository(ctx context.Context, currentResource *sourcev1.HelmRepository, targetSource *sourcev1.HelmRepository) error {
	currentResource.Spec = targetSource.Spec
	if err := a.Client.Update(ctx, currentResource); err != nil {
		return err
	}
...
func (a *ApplicationManager) updateOCIRepository(ctx context.Context, currentResource *sourcev1.OCIRepository, targetSource *sourcev1.OCIRepository) error {
	currentResource.Spec = targetSource.Spec
	if err := a.Client.Update(ctx, currentResource); err != nil {
		return err
	}
